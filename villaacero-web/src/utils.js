...
export function buildProducto(raw, colegioNombre) {
  ...
  const puntosCost = Math.max(100, Math.trunc(Math.floor(precio / 100) * costMultiplier))

  let earnMultiplier = 1.0
  if (nombreUpper.includes("POLERON")) earnMultiplier = 1.5
  else if (nombreUpper.includes("BUZO")) earnMultiplier = 1.2
  const puntosEarn = Math.max(1, Math.trunc(Math.floor(precio / 1000) * earnMultiplier))
