  ...
  async function onAssistantComplete(extraCharge, customization) {
    ...
    if (pendingMethod === "whatsapp") {
      ...
      items.forEach((i) => {
        ...
        if (i.producto.colegio) msg += `  Colegio: ${i.producto.colegio}\n`
        msg += "\n"
      })
      if (customization) {
        msg += `Detalles de ajuste: ${customization}\nCargo extra: ${formatPrice(extraCharge)}\n\n`
      }
      msg += `Total final: ${formatPrice(totalFinal)}\n\n*Nota: tengo 3 días hábiles para cualquier cambio de opinión sobre los ajustes.*`
      openWhatsApp(msg)
    } else {
      openWebpay()
    }
          ...
          <>
            ...
            <div className="stitch-card">
              ...
              <p style={{ fontSize: 12.5, color: "var(--muted)", marginTop: 8 }}>
                Ganarás {pointsToEarn} Villa Puntos con esta compra.
              </p>
            </div>
